...
import { songStartMap, playerMap, voiceChannelMap } from './core/player.js'
import { getQueue, getCurrentSong, isLooping } from './core/queue.js'
import { isAutoplay } from './core/autoplayManager.js'
import { AudioPlayerStatus } from '@discordjs/voice'

app.get('/api/userbot-state', (req, res) => {
  const guildId = req.query.guildId
  if (!guildId) return res.status(400).json({ error: 'guildId required' })

  const player = playerMap.get(guildId)
  let playerStatus = 'disconnected'
  if (player) {
    switch (player.state.status) {
      case AudioPlayerStatus.Playing: playerStatus = 'playing'; break
      case AudioPlayerStatus.Paused:  playerStatus = 'paused';  break
      case AudioPlayerStatus.Idle:    playerStatus = 'idle';    break
    }
  }

  const startTime = songStartMap.get(guildId)
  const elapsed = startTime ? Math.floor((Date.now() - startTime) / 1000) : 0

  const fullQueue = getQueue(guildId)
  const vc = voiceChannelMap.get(guildId)
  const voiceChannelId = vc ? vc.id : null

  res.json({
    guildId,
    currentSong: getCurrentSong(guildId),
    queue: fullQueue.slice(1, 6),   // next 5 songs
    elapsed,
    isLooping: isLooping(guildId),
    isAutoplay: isAutoplay(guildId),
    playerStatus,
    voiceChannelId,
  })
})
